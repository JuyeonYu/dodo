...
class _SearchtodoState extends State<Searchtodo> {
  ...
  void _handleSearch(String searchTerm) {
    ...
    _debounceTimer = Timer(const Duration(milliseconds: 500), () async {
      var test = await firestore.collection('todo').where('title', isGreaterThanOrEqualTo: searchTerm).get();
      setState(() {
        search = searchTerm;
      });
    });
  ...
  Widget build(BuildContext context) {
    ...
    Expanded(
      ...
      if (completedTodos.isEmpty && pendingTodos.isEmpty) {
        ...
        return ListView(
          children: [
            pendingTodos.isEmpty ? Spacer() :
            Helper.BuildSection('Pending', pendingTodos, isSearching: true),
            completedTodos.isEmpty ? Spacer() : Helper.BuildSection('Completed', completedTodos, isSearching: true),
          ],
        );
      }
